    ...
    .then(
        function (response) {
            if (response.status == 200) {response.json().then(data => {
                localStorage.setItem('randomUsers', JSON.stringify(data.results)); //in this case, the objects are wrapped in 'results' - saving locally to use outside of fetch scope.    
                }); 
            } else {
...
function getRandomNotifications() {
    let phrase = [];
    let first = Math.floor(Math.random() * notices.length);
    phrase = notices.splice(first, 1);
    let second = Math.floor(Math.random() * notices.length);
    phrase.push(notices.splice(second, 1));
...
let userList = members.concat(randomUsers); //adding the created list to the random list for extra search names
if (typeof userList == null) {
    alert("error loading member profiles from Server, please refresh your page!");
} else {
...
sendButton.addEventListener('click', function(e){
    ...
    function closeAllLists() {
        popItems = document.getElementsByClassName("pop-up");
